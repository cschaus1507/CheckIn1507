...
export default function Manage() {
  ...
  async function resetYear() {
    const phrase = window.prompt(
      "⚠️ NEW YEAR ROLLOVER\n\nThis will CLEAR:\n- Attendance logs\n- Attendance correction requests\n- Tasks + task comments + assignments\n\nIt will KEEP the student roster.\n\nType RESET to confirm."
    );
    if (phrase !== "RESET") return;

    try {
      await api("/api/admin/reset-year", {
        method: "POST",
        body: JSON.stringify({ confirm: "RESET" })
      });
      setMsg("✅ Year rollover complete. Attendance + tasks cleared.");
      await load();
    } catch (e) {
      setMsg(e.message);
    }
  ...
  return (
    <div className="grid gap-6">
      ...
      <Card title="New Year Rollover">
        <div className="text-sm text-slate-300">
          Clears attendance logs, attendance correction requests, and the entire task board for a fresh season/year.
          <span className="text-slate-200 font-semibold"> Student roster is kept.</span>
        </div>

        <div className="mt-2 text-xs text-slate-500">
          This action is disabled unless the server environment variable{" "}
          <span className="text-slate-300 font-semibold">ALLOW_YEAR_RESET</span> is set to{" "}
        ...
        <button
          type="button"
          onClick={resetYear}
          className="mt-4 px-4 py-3 rounded-xl bg-red-700 hover:bg-red-600 transition font-bold text-white border border-red-500/40"
        >
          Reset for New Year
        </button>

        <div className="text-xs text-slate-500 mt-2">
          You will be prompted to type <span className="text-slate-300 font-semibold">RESET</span> to confirm.
        </div>
      </Card>
